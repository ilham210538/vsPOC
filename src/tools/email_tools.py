...
def read_schedule(
    ...
    try:
        ...
        try:
            ...
            # If we're here, we're in an async context - use create_task
            task = loop.create_task(_read_schedule_async(user, start_iso, end_iso, tz, select, top))
            # Run synchronously by using run_until_complete on a new thread
            import threading
            result = None
            exception = None
            ...
            def run_in_thread():
                ...
                try:
                    ...
                    result = new_loop.run_until_complete(_read_schedule_async(user, start_iso, end_iso, tz, select, top))
                    new_loop.close()
...
def create_meeting(
    ...
    try:
        ...
        try:
            # Try to get the current event loop
            loop = asyncio.get_running_loop()
            # If we're here, we're in an async context - use a new thread
            import threading
            result = None
            exception = None
            ...
            def run_in_thread():
                ...
                try:
                    new_loop = asyncio.new_event_loop()
                    asyncio.set_event_loop(new_loop)
                    result = new_loop.run_until_complete(_create_meeting_async(
                        user,
                        subject,
                        start_iso,
                        end_iso,
